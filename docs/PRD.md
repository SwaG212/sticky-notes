...
flowchart TD
    ...
    G -->|按 Esc| N
    G -->|悬停任务| R[显示时间 + ✕] 【本次更新】
    R -->|点击时间| S[弹出时分选择器] 【本次更新】
    S -->|选时/分| T[保存 → 关选择器] 【本次更新】
    S -->|清除| U[恢复 "--:--"] 【本次更新】
    Q1[定时器每30秒检查] --> Q2{任务时间到达?} 【本次更新】
    Q2 -->|是| Q3[弹出提醒弹窗] 【本次更新】
    Q2 -->|否| Q1 【本次更新】
```
...
│   └── 定时提醒（设置时间，到时弹窗） 【本次更新】
│       ├── 悬停显示时间（✕ 左侧），未设置显示 "--:--"
│       ├── 点击时间 → 弹出时分选择器（两列：小时 0-23 / 分钟 0-59）
│       ├── 选完自动关闭并保存，选择器顶部有"清除"按钮
│       ├── 到达设定时间 → 右下角弹窗，确认按钮手动关闭
│       ├── 多个任务相同时间 → 合并一个弹窗显示
│       ├── 已过时间无视，跨天清空所有时间
│       └── 锁屏状态下也弹窗，应用关闭则无弹窗
...
┌────────────────────────────────────┐
│                                    │
│   ☐ 周三前出合同     13:25 ✕  │  ← 任务清单区（上部方形区域）
│   ☐ 找运维要服务器账号  --:-- ✕  │     悬停时显示时间 + ✕ 按钮
│   ☐ 下午3点开会       --:-- ✕  │     点击时间 → 弹出时分选择器
│   ☐ 给客户回电话      --:-- ✕  │     【本次更新】
│   ☐ 写周报  ☑ 已完成项目  --:-- ✕ │
│                                    │
...
- **定位**：跟随时间文字弹出，定位在文字上方
- **行为**：点击时或分后自动关闭并保存，点击外部也可关闭
- **尺寸**：约 120px 宽，两列各 60px，高度约 180px，内部滚动
...
**功能描述**：用户可为每条任务设置提醒时间（24 小时制，精确到分）。到达设定时间时，在屏幕右下角弹出提醒窗口。支持多个任务相同时间合并提醒、锁屏提醒、跨天自动清空时间。

**触发条件**：
- 时间显示：鼠标悬停任务行时，✕ 按钮左侧显示时间
- 时间选择器：点击时间文字弹出
- 提醒弹窗：系统时间到达用户设定的时分（由定时器每 30 秒检查一次）

**交互细节**：

| 场景 | 交互处理方式 |
|------|------------|
| 鼠标悬停任务行 | ✕ 按钮左侧显示时间文字（HH:MM 或 "--:--"） |
| 鼠标离开任务行 | 时间文字和 ✕ 按钮同时隐藏 |
| 点击时间文字 | 在时间文字上方弹出时分选择器 |
| 选择时/分 | 点击小时或分钟数字后自动关闭选择器并保存 |
| 点击「清除」按钮 | 时间恢复为 "--:--"（等同于未设置） |
| 点击选择器外部 | 关闭选择器，不保存 |
| 到达设定时间 | 主进程弹出提醒窗口，显示任务名称 |
| 点击提醒弹窗「确认」 | 关闭弹窗 |
...
| 属性 | 说明 |
|------|------|
| 结构 | 顶部"清除"按钮 + 下方两列（时/分），各列独立滚动 |
| 小时范围 | 00-23（24 小时制） |
| 分钟范围 | 00, 01, 02, ..., 58, 59（每分钟） |
| 当前选中 | 已设置的时间在列表中高亮 |
| 自动关闭 | 点击时或分数字后立即关闭并保存 |
| 外部点击 | 点击选择器之外的区域关闭，不保存 |
...
|------|------|
| 位置 | 屏幕右下角，主窗口上方 |
| 层级 | `alwaysOnTop`，覆盖包括主窗口在内的所有窗口 |
| 关闭方式 | 仅可通过点击「确认」按钮关闭，不自动消失 |
| 多任务合并 | 同一时间设置多个任务时，合并到一个弹窗，逐行显示每条任务的提醒文案 |
| 锁屏 | 弹窗不被锁屏阻挡，用户解锁后可看到 |
...
|------|------|
| 当天到时间 | 正常弹窗提醒 |
| 时间已过 | 不弹窗（如 15:00 设 13:00，当天不再提醒） |
| 跨天 | 每天加载任务时，所有 `alarmTime` 清空为 `null`（恢复到 "--:--"） |
| 应用关闭 | 应用未运行时不弹窗（无后台服务） |

**状态清单**：

| 状态 | 触发条件 | UI 表现 | 用户可执行操作 |
|------|---------|---------|-------------|
| 未设置 | 任务创建/跨天清空 | 悬停显示 "--:--" | 点击设置时间 |
| 已设置 | 用户设置时间 | 悬停显示 "HH:MM" | 点击修改/清除 |
| 时间已过 | 系统时间 > 设定时间 | 同已设置，但不再触发提醒 | 修改/清除 |
| 提醒弹窗 | 系统时间 == 设定时间 | 弹窗显示任务名 | 点击确认关闭 |

**边界条件**：

- 任务删除时：对应的 alarm 时间随之清除（无意义）
- 设置时间为 00:00 时：有效（午夜提醒）
- 每分钟检查一次：用 `setInterval(30000)` 每 30 秒轮询检查
- 跨天清空：`loadTasks` 时将所有任务的 `alarmTime` 置为 `null`

**数据规范**：

| 字段名 | 数据类型 | 长度 | 是否必填 | 默认值 | 格式 | 校验 |
|--------|---------|------|---------|--------|------|------|
| alarmTime | string 或 null | 5 字符 | 否 | null | "HH:MM" | 00 ≤ HH ≤ 23, 00 ≤ MM ≤ 59 |
...
| 配置保存按钮 | 保存并启动 | 动词 + 结果 |
| 未设置时间 | --:-- | 占位符，表示无提醒 【本次更新】 |
| 提醒弹窗标题 | 便利贴 - 提醒 | 场景明确 【本次更新】 |
| 提醒弹窗内容 | 任务「XXX」的时间到了 | 包含任务名，每条一行 【本次更新】 |
| 提醒确认按钮 | 确认 | 动词，关闭弹窗 【本次更新】 |
| 时间选择器清除 | 清除 | 动词，恢复未设置状态 【本次更新】 |
...
- **定时检查频率**：每 30 秒轮询一次，检查是否有任务 alarmTime 已到达 【本次更新】
